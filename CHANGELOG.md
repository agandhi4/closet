...
- HEIC/HEIF uploads: decoded server-side with heic-decode to raw pixels (capped by `MAX_HEIC_BYTES`), accepted by the photo inputs; browsers that cannot decode HEIC upload the original as it is
...
- Server-side background removal, the only background removal: the phone uploads only the photo, the server queues it and cuts it out with BiRefNet 512x512 in a child process (about 3 s a photo on an 8-core Ryzen, ~3.5 GB of RAM while loaded, needs AVX2 or better), and the garment page shows "Removing background…" until the cutout is swapped in, or "Try again" when it failed (failed ones are retried nightly up to three runs). A cutout edited in the mask editor is never replaced by the server's. The 940 MB model is downloaded into `MODELS_PATH` at boot, checksum-verified; `npm run cutout:fetch-model` seeds or repairs it

#### Fixed

- The phone shrinks a large photo to 1600 px (JPEG) before uploading it: a 24 MP iPhone photo uploaded slowly (and made the in-browser cutout of the time fail). A photo the browser cannot open (HEIC on Android) still goes up as it is
...
- In-browser background removal and `CUTOUT_MODE`: the server makes every cutout. Gone with it: `@imgly/background-removal` (and its patch), `@imgly/background-removal-data` (builds no longer reach staticimgly.com), `onnxruntime-web`, the `/bg-removal-models`, `/modules/background-removal` and `/modules/onnxruntime-web` routes, the "Auto-remove background" toggle, and the CSP's `'unsafe-eval'` and `blob:` scripts, workers and fetches. The service worker deletes the model's caches (~42 MB on a phone that used it). A page the installed app cached before the change may still post the browser's cutout with the photo: it is ignored and the server makes its own. A leftover `CUTOUT_MODE` is ignored; photos stored before keep their browser-made cutouts
