...
src/
  ...
  auth/                Login/register/account controllers (profile, update email, change password,
                       ...
                       SessionGuard + @Public() (the gate for Nest routes), session-access.ts (the
                       decision SessionGuard and the web layer's hook share), @UserId(), RegistrationGuard
  web/                 The plain-Fastify side of the migration (see Web layer): plugin.ts (registered by
                       createApp), auth.ts (session hook), errors.tsx (error page + handler), render.ts,
...
Every request goes through the root preHandler in `app.ts` (session into `req.auth`, `ViewContext` into `reply.locals`; skipped on static paths), then one of two stacks on the same Fastify instance:

- **Web layer (ported routes)**: route handler in `src/web/<feature>/routes.tsx` → the plugin's `requireSession` preHandler → `renderPage(reply, <FeaturePage ctx={viewContext(reply)} />)` or `renderFragment(...)` → JSX components in the shared `Layout`.
...
- **Routes**: `src/web/<feature>/routes.tsx` exports a plugin (`FastifyPluginCallback<WebOptions>`); add `app.register(featureRoutes, options)` to `webPlugin`. Config the routes need is resolved once in `createApp()` into `WebConfig`; nothing in `src/web/` reads `ConfigService` or `process.env`.
- **Auth**: every route needs a session; `{ config: { public: true } }` opts out. `requireSession` (`src/web/auth.ts`) and `SessionGuard` both take `decideSessionAccess` (`src/auth/session-access.ts`), so a page navigation without a session is a 302 to `/auth/login` and an htmx fragment or fetch a bodiless 401 with `HX-Redirect`, on either side. A handler reads the user from `request.auth` (always set past the hook on a non-public route).
- **Queries**: Drizzle, passed in through `WebOptions` when the first feature needs it (`app.get(DB)` in `createApp()`), in `src/web/<feature>/queries.ts`. No MikroORM in ported code.
...
- **Rendering**: `return renderPage(reply, <Page ctx={viewContext(reply)} />, { status })` (adds the doctype) or `renderFragment(...)`; both set `text/html; charset=utf-8` and throw instead of sending twice.
...
- **Web-layer errors before the root preHandler have no page context.** A body that fails to parse (or anything under a static path) reaches the error handler with `reply.locals` unset, so it answers `{ statusCode, message }` with the right status instead of the page.
- **Nest middleware and guards never run for web-layer routes.** Nest registers `@fastify/middie` during `app.init()`, after `webPlugin`, and a Fastify plugin inherits only the hooks its parent had when the plugin was registered. So nestjs-pino's request log (hence the plugin's own `onResponse` line), nestjs-i18n's language resolution (`reply.locals.locale` is always `en` there) and `ThrottlerGuard` do not apply. Root hooks added in `createApp()` before the plugin (session, security headers, cookies, compression, multipart) do. Throttling lost nothing: `ThrottlerModule.forRoot()` has no throttlers (the boot log warns "no request is limited"), so even login's `@Throttle` limits nothing today; a ported login needs a real limiter.
