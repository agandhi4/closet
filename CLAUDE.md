...
src/
  ...
  dal/                 MikroORM, for queries not yet ported
    ...
    entity/            user, garment, file, shareableId: what the unported garment, file and
                       share-page code still queries. Outfits, outfit_slot, outfit_calendar,
                       wardrobe_share and user_device have none (Drizzle only). Kept in step with
...
Every photo is a set of WebP files sharing one base name: `<uuid>.webp` (original, 1080px, q90), `<uuid>-nobg.webp` (browser-made cutout, same size), `<uuid>-thumb.webp` (400px, q80, derived from the cutout when present). Only the original has a `File` row; `File.version` is bumped whenever bytes under an existing name are rewritten (a mask edit), and every variant URL carries `?v=<version>` so the `/file` routes (`src/web/files/routes.ts`) serve all three immutable for a year. Templates never build `/file/...` by hand: `{{imageUrl photo 'thumb'}}` (grids and strips, with `loading="lazy"` and dimensions) or `'nobg'` (detail, share, Open Graph). Thumbs missing in storage are generated on first request, single-flighted, which backfills old photos. Deletion always goes through `deleteVariants`.

All of it is `Photos` (`src/web/files/photos.ts`, plain TypeScript, Drizzle for the `file` table) over `PhotoStorage` (`storage.ts`: a flat directory, `DATA_PATH`, the only storage; atomic writes through `.incoming/`). One instance per process, built by `createPhotos()`: the thumb single-flight map lives in it, so the web layer and the Nest garment code must share it (`FileModule` provides it, `createApp()` hands the same one to `webPlugin` as `photos`). Uploads (`storeUpload`), copies (`copy`) and cutouts (`storeCutout`) write bytes only and return the `file` row to insert (`NewPhotoRow`); see Gotchas for the commit contract. Failures are `HttpError`s (400 unreadable, 400 "Image too large", 413 HEIC over the byte cap, 404 missing original), which both the web error handler and `ErrorViewFilter` answer with their status.
...
- **A photo upload builds its thumb once, at the end.** `storeUploadedPhotoWithCutout` stores both halves (`storeUpload` with `deferThumb`, `storeCutout` with `newUpload`) and then calls `regenerateThumb`; the mask edit path (`newUpload: false`) still rewrites the thumb before bumping the version.
