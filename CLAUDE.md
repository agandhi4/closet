...
- **Views**: server-rendered HTML with htmx 2 for interactivity and `_hyperscript` for client logic. Not a SPA. There is no JSON API for the UI. Every feature page is typed JSX (`hono/jsx`, rendered to strings, no Hono server) in the plain-Fastify web layer (`src/web/`, see Web layer); since the garment port (2026-09-26) no feature route is in Nest. Handlebars (`@fastify/view`) renders only Nest's own error page (`views/error.hbs` in `views/layout.hbs`, for paths no route matches) until Nest goes.
...
- **i18n**: English only, strings in `src/i18n/en/lang.json` (the other five languages were deleted on 2026-09-26). Handlebars views translate through `nestjs-i18n`; JSX views are English only, `t('KEY')` from `src/web/i18n.ts`, typed to the keys of `src/i18n/en/lang.json`.
- **Logging**: `nestjs-pino`, pretty to stdout and rotating `app.log` under `DATA_PATH`.
- **Tests**: three tiers. Vitest unit (`src/**/*.spec.ts`, mocks everything, verifies wiring); Vitest integration (`test/integration/`, the real app in-process on a scratch Postgres database per spec file, driven through `app.inject()`, verifies behavior: HTML, headers, rows, files); Playwright e2e (`test/*.spec.ts`, real browser against a built server). Both Vitest tiers are projects of one `vitest.config.ts` (unplugin-swc for Nest's decorator metadata; specs import `describe`/`it`/`expect`/`vi` from `'vitest'`, no globals). Integration specs assert rows through `t.db` (Drizzle). `recordQueries(work)` (harness) counts the statements and rows a request reads, for proving a page reads what it shows. Plus autocannon load test (`scripts/load-test.ts`) and Lighthouse CI. Vitest replaced Jest on 2026-09-25, the first step of a platform migration: every feature is now plain Fastify on Drizzle (MikroORM gone 2026-09-26); removing NestJS itself is the last step.
...
src/
  ...
  web/                 The plain-Fastify side of the migration (see Web layer): plugin.ts (registered by
                       ...
                       requireSession; sharing/: /wardrobe-share/*, the access resolver; push/:
  ...
  maintenance/         reconcile.ts (reconcileStorage: storage and the file table kept in step, with
                       ...
                       by main.ts when MAINTENANCE_ENABLED), reconcile.cli.ts (`npm run
                       maintenance:reconcile`). set-password.cli.ts (`npm run user:set-password --
                       <email>`): the locked-out recovery
...
views/                 Handlebars, only Nest's error page now: error.hbs, layout.hbs, partials/ (navbar,
                       dock, app_status: twins of src/web/layout/)
...
- **Nest**: no feature controllers remain. A path no route matches is Nest's 404, rendered by `ErrorViewFilter` as `views/error.hbs`; `SessionGuard`, `ViewContextService`, `DbModule` and the CLIs' application context are the Nest infrastructure a later step removes.
...
- **Queries**: Drizzle, `options.db` (`app.get(DB)` in `createApp()`), in `src/web/<feature>/queries.ts`. A handler reads the user id with `sessionUserId(request)` (`src/web/auth/require-session.ts`, the twin of `@UserId()`). Row assertions go through `t.db`.
...
- **Errors**: `throw new HttpError(404)` (`src/web/errors.tsx`). The plugin's error handler renders `ErrorPage` in the layout with the status, as `ErrorViewFilter` does for Nest: 4xx keep their message, anything else is a logged 500 without detail. A Nest `HttpException` maps to its status too.
...
- **Shared shell**: `views/layout.hbs` and the navbar, dock and app_status partials have JSX twins in `src/web/layout/`. Nest's error page (unmatched paths) still renders the Handlebars shell, so until Nest goes a change to the shell goes into both.
...
- **Every user-facing string goes through i18n.** `t('KEY')` in JSX views, `{{t 'lang.KEY'}}` in Handlebars templates, `i18n.t()` in Nest controllers and DTO validation messages. Add the key to `src/i18n/en/lang.json`, the only catalog.
...
npm run maintenance:reconcile [-- --dry-run] [--force]
                              ...
                              # guard refused; --force deletes anyway (dry-run first)
...
`src/db/schema.ts` is the schema; `drizzle/` holds the migrations drizzle-kit generates from it. The app migrates on every boot (`DbModule.onModuleInit`: a global module, so before every other module's init and before anything queries), so a deploy is the migration.
...
- **The request logger redacts `cookie`, `authorization` and `set-cookie`, and skips static paths.** It is pino-http, which since the garment port only sees requests Nest answers (paths no route matches); web-layer routes log their own header-free line. The session cookie is a year-long bearer credential and the logs reach `app.log` and Loki. `autoLogging.ignore` must read `originalUrl`: Nest mounts pino-http as middleware, which strips the mount prefix, so `req.url` is `/` for every request.
...
- **Only the server schedules the nightly reconciliation.** `main.ts` starts `scheduleNightly` (03:00 in `APP_TIMEZONE`, recomputed after every run, never overlapping) when `MAINTENANCE_ENABLED`; `createApp()` does not, so the integration harness and the CLIs (which boot the same modules) never hold a timer. `@nestjs/schedule` is gone.
...
- **Web-layer errors before the root preValidation hook have no page context.** A body that fails to parse reaches the error handler with `reply.locals` unset, and anything under a static path (`/file/**`) is recognised by its path, so both answer `{ statusCode, message }` with the right status instead of the page. The path check is needed because nestjs-i18n's root preHandler sets `reply.locals = {}` on every routed request, static ones included: `reply.locals` being truthy does not mean the page context exists. A schema validation failure comes after the hook and gets the 400 page. The hook must stay at preValidation (not preHandler): validation runs between the two.
