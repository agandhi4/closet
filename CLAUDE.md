...
- **Routes**: `GET /wardrobe` (the page, or `#wardrobe-main` alone to an htmx fragment request, with `Vary`), `GET /wardrobe/tiles` (the next grid page; always a fragment), `GET /wardrobe/new`, `GET /wardrobe/:id`, `GET /wardrobe/:id/edit`, `GET|POST /wardrobe/:id/clone`, `POST /wardrobe`, `POST /wardrobe/:id` (every field; a cleared one is NULL), `POST /wardrobe/:id/photo` (multipart `photo` + optional `nobgPhoto`, `HX-Redirect` to `?photoSaved=1`), `POST /wardrobe/:id/nobg` (the mask editor's fetch, answers `{ version }`), `POST /wardrobe/:id/archive` and `DELETE /wardrobe/:id` (`HX-Redirect`, owner only).
...
- **Wardrobe fragment.** `GET /wardrobe` with a fragment request (`HX-Request` without `HX-Boosted`/history restore) returns `WardrobeMain` (`#wardrobe-main`) with `Vary`; the filter bar and search form target `#wardrobe-main` with `hx-push-url`, so filtering never re-renders navbar and dock. The grid's later pages come from `GET /wardrobe/tiles`, always htmx requests, so the worker caches each under its own `url|hx` key; offline, the pages already scrolled through are what the cache has.
